...
from .Items import BombTSAItem, item_data_table, item_table
from .Locations import *
from .Options import BombTSAOptions
...
class BombTSAWebWorld(WebWorld):
    ...
    setup_de = Tutorial(
        ...
        description="Eine Anleitung um Bomberman The Second Attack zu spielen.",
        language="Deutsch",
    ...
    def create_regions(self) -> None:
        ...
        for region_name, region_data in region_data_table.items():
            ...
            if self.options.shopsanity:
                region.add_locations({
                location_name: location_data.address for location_name, location_data in shop_data_table.items()
                if location_data.region == region_name and location_data.can_create(self)# and (region_name in self.included_stages or region_name in fixed_regions)
                }, BombTSALocation)
                
            if self.options.pommysanity:
                region.add_locations({
                location_name: location_data.address for location_name, location_data in pommy_data_table.items()
                if location_data.region == region_name and location_data.can_create(self)# and (region_name in self.included_stages or region_name in fixed_regions)
                }, BombTSALocation)
            
            if self.options.powersanity:
                region.add_locations({
                location_name: location_data.address for location_name, location_data in powerup_data_table.items()
                if location_data.region == region_name and location_data.can_create(self)# and (region_name in self.included_stages or region_name in fixed_regions)
                }, BombTSALocation)
